...
class L_BFGS_B:
    ...
    def update_loss_weights(self):
        """
        Dynamically update loss weights based on the latest component losses.
        Strategy: Inverse loss magnitude normalization.
        """
        ...
        recent_losses = np.clip(recent_losses, 1e-8, None)
        inv_losses = 1.0 / recent_losses
        normed_weights = inv_losses / np.sum(inv_losses)

        self.loss_weights = normed_weights.tolist()
        print(f"Updated loss weights: {np.round(self.loss_weights, 4)}")
