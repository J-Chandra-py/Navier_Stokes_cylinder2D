...
class PINN:
    ...
    def build(self):
        ...
        xy_roi = tf.keras.layers.Input(shape=(2,))
        # boundary condition
        ...
        p_grads, u_grads, v_grads = self.grads(xy_roi)
        _, p_x, p_y = p_grads
        u, u_x, u_y, u_xx, u_yy = u_grads
        v, v_x, v_y, v_xx, v_yy = v_grads
        # compute equation loss
        ...
        v_eqn =  u*v_x + v*v_y + p_y/self.rho - self.mu*(v_xx + v_yy) / self.rho
        uv_eqn = u_x + v_y
        uv_eqn_roi = tf.concat([u_eqn, v_eqn, uv_eqn], axis=-1)
        ...
        return tf.keras.models.Model(
            inputs=[xy_eqn, xy_roi, xy_w1, xy_w2, xy_out, xy_in, xy_circle], outputs=[uv_eqn_int, uv_eqn_roi, uv_in, uv_out, uv_w1, uv_w2, uv_circle]) # tensor order w.r.t original code