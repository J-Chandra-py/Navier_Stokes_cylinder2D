...
class L_BFGS_B:
    ...
    def update_loss_weights(self):
        """
        Dynamically update loss weights using log-scale softmax-style normalization.
        More stable across loss magnitudes.
        """
        if not self.individual_loss_history:
            return  # No data to update from

        recent_losses = np.array(self.individual_loss_history[-1])
        recent_losses = np.clip(recent_losses, 1e-8, 1e2)  # Prevent zero or large outliers

        # Log scale to damp large variation
        log_losses = np.log(recent_losses)
        log_losses -= np.mean(log_losses)

        weights = np.exp(-log_losses)  # lower loss → higher weight
        weights /= np.sum(weights)     # normalize

        self.loss_weights = weights.tolist()
        print(f"[ADAPTIVE] Loss weights updated: {np.round(self.loss_weights, 4)}")
    ...
    def callback(self, weights):
        ...
        # Rebalance every 20 iterations (adjustable)
        if len(self.loss_history) % 20 == 0:
